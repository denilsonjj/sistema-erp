...
import { BridgeIcon, CubeIcon, PlusIcon, TrashIcon, ChartIcon, UserGroupIcon, BookOpenIcon, SunIcon, CloudIcon, CloudRainIcon, ShoppingCartIcon, GraderIcon, XMarkIcon, PrinterIcon, BuildingIcon, BriefcaseIcon } from './icons';
...
const MaterialModal = ({ isOpen, onClose, onSave, projectId }) => {
    const [data, setData] = useState({
        receiptDate: new Date().toISOString().split('T')[0],
        supplier: '', material: '', quantity: '', unit: 'un', unitPrice: '', freightValue: '', docNumber: '', docType: 'NF'
    });
    if (!isOpen)
        return null;
    const handleSubmit = () => {
        if (data.material && data.quantity && data.unitPrice) {
            ...
            setData({ receiptDate: new Date().toISOString().split('T')[0], supplier: '', material: '', quantity: '', unit: 'un', unitPrice: '', freightValue: '', docNumber: '', docType: 'NF' });
            onClose();
        }
    };
    return (<div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
            <div className="bg-brand-secondary p-6 rounded-2xl w-full max-w-lg border border-slate-700 shadow-2xl max-h-[90vh] overflow-y-auto">
                <h3 className="text-lg font-bold text-brand-light mb-4">Registrar Entrada de Material</h3>
                <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2"><label className="text-xs text-brand-muted font-bold">Material</label><input type="text" value={data.material} onChange={e => setData({ ...data, material: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"/></div>
                    <div className="col-span-2"><label className="text-xs text-brand-muted font-bold">Fornecedor</label><input type="text" value={data.supplier} onChange={e => setData({ ...data, supplier: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"/></div>
                    ...
                    <div><label className="text-xs text-brand-muted font-bold">Nota Fiscal / Doc</label><input type="text" value={data.docNumber} onChange={e => setData({ ...data, docNumber: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"/></div>
                    ...
                    <div><label className="text-xs text-brand-muted font-bold">Unidade</label><select value={data.unit} onChange={e => setData({ ...data, unit: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"><option value="un">Unidade</option><option value="kg">Kg</option><option value="m">Metro</option><option value="m3">m³</option><option value="cx">Caixa</option><option value="saco">Saco</option></select></div>
                    ...
                    <div><label className="text-xs text-brand-muted font-bold">Frete (R$)</label><input type="number" step="0.01" value={data.freightValue} onChange={e => setData({ ...data, freightValue: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"/></div>
                </div>
                <div className="flex justify-end gap-2 mt-6 border-t border-slate-700 pt-4">
                    <button onClick={onClose} className="px-4 py-2 text-brand-muted font-bold hover:text-white">Cancelar</button>
                    <button onClick={handleSubmit} className="bg-teal-600 text-white px-6 py-2 rounded-lg font-bold text-xs hover:bg-teal-500 uppercase">Salvar Material</button>
                </div>
            </div>
        </div>);
};
const DailyLogModal = ({ isOpen, onClose, onSave, projectId, machines }) => {
    ...
    const handleAddEquip = () => {
        if (selectedMachine && equipCost) {
            ...
            if (machine) {
                setEquipList([...equipList, { prefix: machine.prefix, dailyCost: parseFloat(equipCost) }]);
                setSelectedMachine('');
                setEquipCost('');
            }
        }
    ...
    return (<div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
            <div className="bg-brand-secondary p-6 rounded-2xl w-full max-w-2xl border border-slate-700 shadow-2xl max-h-[90vh] overflow-y-auto">
                <h3 className="text-lg font-bold text-brand-light mb-4">Novo Diário de Obra</h3>
                <div className="grid grid-cols-2 gap-4 mb-4">
                    <div><label className="text-xs text-brand-muted font-bold">Data</label><input type="date" value={logData.date} onChange={e => setLogData({ ...logData, date: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"/></div>
                    <div><label className="text-xs text-brand-muted font-bold">Clima</label><select value={logData.weather} onChange={e => setLogData({ ...logData, weather: e.target.value })} className="w-full bg-brand-primary border border-slate-600 text-brand-light rounded p-2 outline-none"><option value="Sol">Sol</option><option value="Nublado">Nublado</option><option value="Chuva">Chuva</option><option value="Impraticável">Impraticável</option></select></div>
                </div>
                <div className="mb-4">
                    <label className="text-xs text-brand-muted font-bold">Descrição das Atividades</label>
                ...
                <div className="bg-brand-primary p-3 rounded-lg border border-slate-700 mb-4">
                    ...
                    {equipList.length > 0 && (<div className="space-y-1 mt-2">
                            {equipList.map((eq, idx) => (<div key={idx} className="flex justify-between text-xs bg-slate-700/50 px-2 py-1 rounded">
                ...
                <div className="flex justify-end gap-2 border-t border-slate-700 pt-4">
                    <button onClick={onClose} className="px-4 py-2 text-brand-muted font-bold hover:text-white">Cancelar</button>
                    <button onClick={handleSubmit} className="bg-amber-600 text-white px-6 py-2 rounded-lg font-bold text-xs hover:bg-amber-500 uppercase">Salvar Diário</button>
                </div>
            </div>
        </div>);
};
// --- COMPONENTE PRINCIPAL ---
const PontesView = ({ projects, materials, withdrawals, employees, fixedCosts, machines, dailyLogs, workers, onAddProject, onAddMaterial, onDeleteMaterial, onAddEmployee, onEditEmployee, onDeleteEmployee, onUpdateEmployeeStatus, onSaveFixedCosts, onSaveDailyLog }) => {
    ...
    const projectTotals = useMemo(() => {
        ...
        const labCost = projectEmployees.reduce((acc, curr) => {
            ...
            // Custos de alimentação só são somados se NÃO estiver em baixada ou demitido no status atual
            // Para funcionários demitidos, o custo é fixo até a data de demissão (já calculado em days) + acerto
    ...
    return (<div className="space-y-6">
                ...
                return (<tr key={emp.id} className={`hover:bg-slate-700/50 ${isDismissedPending ? 'bg-yellow-500/5' : ''}`}>
                    ...
                    <div className="bg-brand-secondary p-6 rounded-lg shadow-lg border-t-4 border-amber-500">
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="text-lg font-semibold text-brand-light flex items-center gap-2"><BookOpenIcon className="w-6 h-6 text-amber-400"/> Diário de Obras</h4>
                            <button onClick={() => setIsDailyLogModalOpen(true)} className="bg-amber-600 text-white px-4 py-2 rounded-lg text-xs font-black uppercase shadow-lg hover:bg-amber-500 transition-all flex items-center gap-2"><PlusIcon className="w-3 h-3"/> Novo Registro</button>
                        </div>
                ...
                <div className="space-y-6">
                    <div className="bg-brand-secondary p-6 rounded-lg shadow-lg border-t-4 border-teal-500 h-full flex flex-col">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setMaterialModalOpen(true)} className="bg-teal-600 text-white p-2 rounded-lg hover:bg-teal-500 shadow-lg transition-all" title="Adicionar Material"><PlusIcon className="w-4 h-4"/></button>
                        </div>
                            ...
                            {projectMaterials.map(m => (<div key={m.id} className="bg-brand-primary p-3 rounded-lg border border-slate-700 group hover:border-teal-500/30 transition-colors">
                                    <div className="flex justify-between items-start">
                                        <div className="flex-1">
                                            <p className="font-bold text-brand-light text-xs line-clamp-1">{m.material}</p>
                                            <p className="text-[10px] text-brand-muted">{m.supplier} • {new Date(m.receiptDate + 'T00:00:00').toLocaleDateString('pt-BR')}</p>
                                        ...
                                        <div className="flex flex-col items-end gap-1">
                                            ...
                                            <button onClick={() => onDeleteMaterial(m.id)} className="text-slate-600 hover:text-red-500 transition-colors"><TrashIcon className="w-3 h-3"/></button>
                                        </div>
                                    </div>
                                </div>))}
      ...
      {selectedProjectId && currentProject && (<>
            <MaterialModal isOpen={isMaterialModalOpen} onClose={() => setMaterialModalOpen(false)} onSave={onAddMaterial} projectId={selectedProjectId}/>
            ...
            <DailyLogModal isOpen={isDailyLogModalOpen} onClose={() => setIsDailyLogModalOpen(false)} onSave={onSaveDailyLog} projectId={selectedProjectId} machines={machines}/>
